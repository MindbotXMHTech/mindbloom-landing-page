...
import { loadPublicServiceCards } from "../../lib/serviceContent";
import {
  workshopCategoriesFallback,
  type PublicWorkshopCategory,
} from "../../constants/workshopData";
import { loadPublicWorkshopCategories } from "../../lib/workshopContent";
...
function buildClinicServiceSections(workshopTags: string[]): ClinicServiceSection[] {
  return [
  {
    id: "assessment",
    title: {
...
function ServicePage() {
  const { t } = useLanguage();
  const [serviceCards, setServiceCards] = useState<PublicServiceCard[]>(serviceCardsFallback);
  const [workshopCategories, setWorkshopCategories] = useState<PublicWorkshopCategory[]>(
    workshopCategoriesFallback,
  );
  ...
  useEffect(() => {
    ...
    const loadContent = async () => {
      const [nextCards, nextCategories] = await Promise.all([
      ...
      setWorkshopCategories(nextCategories);
    };
  ...
  const clinicServiceSections = useMemo(
    () => buildClinicServiceSections(workshopCategories.map((category) => category.title.en)),
    [workshopCategories],
  );
              ...
              <div className="relative mt-6 h-28 w-full text-[#e5aba4]">
                <div className="absolute top-0 left-6 max-w-32.5 s:max-w-41.25">
                  <p className="text-[40px] s:text-[32px] font-bold leading-tight wrap-break-word">
                    {t({
                      th: card.durationLines[0]?.th ?? "",
                      en: card.durationLines[0]?.en ?? "",
                    })}
                  </p>
                </div>
                <img
